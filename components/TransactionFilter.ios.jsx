/**
 * TransactionFilter — iOS
 *
 * Filter button: SwiftUI glass Button (borderedProminent when active, glass when inactive).
 * Modal body: NativeBottomSheet.ios.jsx → native SwiftUI sheet with system glass background.
 *             Uses BottomSheetScrollView so the content scrolls correctly within the sheet.
 */
...
const TransactionFilter = ({
  ...
  const addDateGroup = () => {
    const newId = makeId();
    updateStaged('dateRanges', [
      ...(stagedFilters.dateRanges || DEFAULT_DATE_RANGES),
      { id: newId, type: 'all', start: null, end: null },
    ]);
  ...
  const deleteActiveDateGroup = () => {
    ...
    Alert.alert('Delete Date Group', `Delete "${label}"?`, [
  ...
  const addAmountGroup = () => {
    const newId = makeId();
    updateStaged('amountRanges', [
      ...(stagedFilters.amountRanges || DEFAULT_AMOUNT_RANGES),
  ...
  const deleteActiveAmountGroup = () => {
    ...
    Alert.alert('Delete Amount Group', `Delete "${label}"?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete', style: 'destructive',
        onPress: () => {
  ...
  const toggleCategory = (categoryName) => {
    ...
    updateStaged('categories', current.includes(categoryName)
  ...
  const toggleTag = (tagName) => {
    ...
    updateStaged('tags', current.includes(tagName)
  ...
  return (
    <>
      {/* ── iOS: SwiftUI glass filter button ── */}
      <Host matchContents>
        <Button
          label="Filter"
      ...
      >
        ...
        {/* Scrollable content — plain ScrollView works inside BottomSheetView */}
        <ScrollView
                    ...
                    {dateRanges.map((dr, idx) => {
                      ...
                      return (
                        <Animated.View key={dr.id} entering={FadeIn.duration(250)} exiting={FadeOut.duration(200)} layout={LinearTransition.duration(250)}>
                          ...
                          >
                            <Text className={`font-sansMed text-sm ${isActive ? 'text-white' : 'text-textMuted'}`}>{label}</Text>
                          </TouchableOpacity>
                        </Animated.View>
                      );
                    })}
              ...
              <ScrollView nestedScrollEnabled showsVerticalScrollIndicator={false}>
                {activeFilterTab === 'categories' ? (
                  filteredCategories.map((item) => (
                    <Animated.View key={item.category_id} entering={FadeIn.duration(250)} exiting={FadeOut.duration(200)} layout={LinearTransition.duration(250)}>
                      <TouchableOpacity
                      ...
                      >
                        <Ionicons name={item.icon} size={20} color={stagedFilters.categories?.includes(item.category) ? 'white' : themeColors.primary} />
                  ...
                  filteredTags.map((item) => {
                    ...
                    return (
                      <Animated.View key={item.tag_id} entering={FadeIn.duration(250)} exiting={FadeOut.duration(200)} layout={LinearTransition.duration(250)}>
                        <TouchableOpacity
                        ...
                        >
                          <Text className="font-sansMed text-lg text-textMain">{item.tag_name}</Text>
                        </TouchableOpacity>
                      </Animated.View>
                    );
                  })
        ...
        {/* Footer actions — pinned outside the scroll */}
        <View style={{ gap: 12, paddingTop: 12 }}>
          ...
          <TouchableOpacity onPress={handleApply} className="bg-primary py-4 rounded-2xl items-center">
